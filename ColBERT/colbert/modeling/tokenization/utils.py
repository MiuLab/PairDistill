...
def tensorize_triples(query_tokenizer, doc_tokenizer, queries, passages, scores, bsize, nway, pair_scores=None):
    ...
    if pair_scores is not None and len(pair_scores):
        pair_score_batches = _split_into_batches2(pair_scores, bsize)
    else:
        pair_score_batches = [[] for _ in query_batches]

    batches = []
    for Q, D, S, P in zip(query_batches, doc_batches, score_batches, pair_score_batches):
        batches.append((Q, D, S, P))
