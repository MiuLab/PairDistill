...
class RunSettings:
    ...
    name: str = DefaultVal(timestamp(daydir=False))
...
class TrainingSettings:
    ...
    temperature: float = DefaultVal(1.0)

    ignore_scores: bool = DefaultVal(False)

    model_name: str = DefaultVal(None) # DefaultVal('bert-base-uncased')

    rank_loss: str = DefaultVal(None)

    ranknet_weight_type: str = DefaultVal(None)  # 'diff' or 'diff_powed'

    use_pair_scores: bool = DefaultVal(False)

    pair_loss_alpha: float = DefaultVal(1.0)

    only_pair_loss: bool = DefaultVal(False)
